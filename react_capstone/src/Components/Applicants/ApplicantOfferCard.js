...
const ApplicantOfferCard = (props) => {
        ...
        return ( <
                ...
                div >
                <
                Card.Title > { job_title } - < span className = "text-muted font-weight-light" > { er_name } < /span> <
                /Card.Title> <
                Card.Subtitle className = "text-muted mb-2" > { day + "/" + month + "/" + year } <
                /Card.Subtitle> { offer ? < p className = "flex"
                    style = {
                        { backgroundColor: "green" } } > { String(offer) } < /p>: <p></p > } <
                Badge className = "job-list-badge"
                variant = "secondary" > { job_type } < /Badge> <
                Badge className = "job-list-badge"
                variant = "secondary" > { job_location } < /Badge>

                <
                /div> {
                    er_img_data ? < img className = "d-none d-md-block"
                    height = "100"
                    src = { er_img_data }
                    alt = "test" / >: < p > < /p>} <
                        /div> <
                    ...
                    fade = { false } >
                        <
                        ModalHeader toggle = { toggle } > { job_title } { er_name }
                    ID: { job_id } < /ModalHeader> <
                        ModalBody style = {
                        ...
                        /ModalBody> <
                        ModalFooter > {
                            reply === null && offer ? < Button color = "primary"
                            onClick = {
                                () => handleAccept() } > accept < /Button> : null } {
                                reply === null && offer ? < Button color = "primary"
                                onClick = {
                                        () => handleDecline() } > decline < /Button> : null } <
